	...
	if( isDlpAlgo )
		...
		if( cryptStatusOK( status ) )
			...
			if( *dataPtr == 0 )
				...
				REQUIRES( rangeCheck( dataLength, 1, wrappedDataLength ) );
...
static int generatePkcs1DataBlock( INOUT_BUFFER( dataMaxLen, *dataLength ) \
	...
	*dataLength = 0;
	...
	ENSURES( pkcs1PadSize >= 8 && \
			 messageLen + pkcs1PadSize + 3 <= dataMaxLen );
...
static int recoverPkcs1DataBlock( IN_BUFFER( dataLength ) const BYTE *data, 
	...
	*pkcs1PadSize = 0;
	...
	/* Undo the PKCS #1 padding:
	   ...
	   values).  Note that some implementations may have bignum code that 
	   zero-truncates the result which will produce a CRYPT_ERROR_BADDATA 
	   error when we undo the padding, it's the responsibility of the lower-
	   level crypto layer to reformat the data to return a correctly-
	...
	result |= ch0pos < 11;						/* Not constant-time */
	}
#else
	...
	/* Sanity check to make sure that the padding data looks OK.  We only do 
	   this in debug mode since it's a probabilistic test and we don't want 
	   to bail out due to a false positive in production code (this will also
	   catch some cases of an attacker trying to use us as an oracle by
	   sending fixed-format padding) */
...
static int pkcs1Unwrap( INOUT MECHANISM_WRAP_INFO *mechanismInfo,
	...
	const int maxPayloadLength = ( type == PKCS1_WRAP_RAW ) ? \
...
static int recoverOaepDataBlock( OUT_BUFFER( messageMaxLen, *messageLen ) \
	...
	REQUIRES( isHashAlgo( hashAlgo ) );
	REQUIRES( isShortIntegerRangeNZ( hashParam ) );
	...
	*messageLen = 0;
	...
	/* Verify that:
	   ...
	   is make it as hard as possible to get timing data, but in any case 
	   see the comment in the PKCS #1 processing code for the low 
	   probability of this occurring.  Not to mention the fact that the OAEP
	   code isn't used anywhere in cryptlib */
