	...
	if( cryptStatusError( status ) )
		...
		if( isErrorTest )
			...
			if( !isServer && status != CRYPT_ERROR_SIGNATURE && \
				...
				{
				printf( "Test returned status %d, should have been %d or %d.\n", 
						status, CRYPT_ERROR_SIGNATURE, CRYPT_ERROR_BADDATA );
				return( status );
				}
...
static int connectSSH( const CRYPT_SESSION_TYPE sessionType,
	...
	if( isServer )
		...
		if( cryptStatusOK( status ) && \
			...
			if( cryptStatusError( status ) )
				{
				printf( "SVR: Pubkey auth.keyset open failed with error "
						"code %d, line %d.\n", status, __LINE__ );
				return( FALSE );
				}
			...
			if( testType == SSH_TEST_PUBKEYAUTH || \
				...
				if( cryptStatusOK( status ) && \
					...
					/* For the pubkey auth we have to provide the same name 
					   ...
					   certificate or the auth will be rejected */
...
dualThreadContinue:
	...
	if( testType == SSH_TEST_PUBKEYAUTH_WRONGKEY || \
		testType == SSH_TEST_PUBKEYAUTH_WRONGNAME )
		{
		printf( "Attempt to connect with invalid key or user name "
	...
	if( testType == SSH_TEST_MULTICHANNEL || \
		...
		if( isServer )
			...
			if( cryptStatusError( status ) )
				{
				printf( "Couldn't close second SSH channel, status %d, line "
						"%d.\n", status, __LINE__ );
				return( FALSE );
				}
	...
	if( isErrorTest )
		{
		puts( "  (This test should have led to a failure but didn't, test "
			  "has failed).\n" );
		return( FALSE );
		}
