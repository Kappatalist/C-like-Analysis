	...
	*userAuthInfo = USERAUTH_ERROR;
	...
	if( methodInfoPtr->type == METHOD_PUBKEY )
		...
		/* In yet another variant of SSH's Lucy-and-Charlie-Brown 
		   ...
		   If we're given a pubkey auth message with the signature missing, 
		   we have to send back a copy of the public key algorithm name and
		   data (presumably to confirm that the three copies the client sent 
		   us arrived in good order), however nothing seems to check this so 
		   we don't bother jumping through all the hoops and just send back 
		   an easter egg */
		if( !kludgeFlag )
			{
