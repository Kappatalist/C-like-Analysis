...
static int getSessionErrorInfo( INOUT NET_STREAM_INFO *netStream, 
	...
	if( cryptStatusOK( status ) )
		{
		setErrorString( NETSTREAM_ERRINFO, errorString, 
						msgData.length );
		}
	...
	if( protocol == STREAM_PROTOCOL_HTTP )
		...
		if( cryptStatusOK( status ) )
			...
			if( connectInfo->options == NET_OPTION_HOSTNAME )
				SET_FLAG( netStream->nFlags, STREAM_NFLAG_HTTPPROXY );
			else
				SET_FLAG( netStream->nFlags, STREAM_NFLAG_HTTPTUNNEL );
...
static int completeConnect( INOUT STREAM *stream,
	...
	if( useTransportBuffering || urlInfo != NULL )
		...
		if( urlInfo != NULL )
			...
			REQUIRES_S( boundsCheckZ( startOffset, urlInfo->hostLen, 
