...
static int readRequestHeader( INOUT STREAM *stream, 
	...
	REQUIRES( ( TEST_FLAG( netStream->nFlags, STREAM_NFLAG_HTTPGET ) && \
				uriInfo != NULL ) || \
			  ( !TEST_FLAG( netStream->nFlags, STREAM_NFLAG_HTTPGET ) && \
				uriInfo == NULL ) );
	...
	*flags = HTTP_FLAG_NONE;
	...
	LOOP_MED( i = 0, httpReqInfo[ i ].reqName != NULL && \
		...
		if( TEST_FLAG( netStream->nFlags, reqInfoPtr->reqTypeFlag ) && \
...
static int readResponseHeader( INOUT STREAM *stream, 
	...
	*flags = HTTP_FLAG_NONE;
	...
	if( TEST_FLAG( netStream->nFlags, STREAM_NFLAG_HTTPGET ) )
		{
...
void setStreamLayerHTTP( INOUT NET_STREAM_INFO *netStream )
	...
	SET_FLAG( netStream->nFlags, STREAM_NFLAG_HTTPPOST );
