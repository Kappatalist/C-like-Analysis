...
/* embOS/IP doesn't have a IPPROTO_TCP or AF_UNSPEC, however it's only used 
   by the IPv6 emulation layer so we can set it to anything we like (embOS
   defines PF_UNSPEC to AF_UNSPEC, but forgets to define AF_UNSPEC) */
...
/* gethostbyname is a problem function because the standard version is non-
   ...
   In addition, Telit and embOS barely have any DNS functionality, and 
   unlike all their other BSD-equivalent functions provide a 
