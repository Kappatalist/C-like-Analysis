...
/* cryptlib's separation kernel causes some problems with objects that use
   ...
   A second problem occurs because when a thread is blocked in an object 
   waiting on a socket there's no way to unblock it apart from killing the 
   thread (actually we could create some sort of lookback socket and wait 
   ...
   For now we limit the socket pool to a maximum of 128 sockets (8 in
   ...
   socket pool.  However it's not a good idea to simply remove the 
...
/* Open a connection to a remote server.  The connection-open function 
   ...
   to play with in most cases we could at least perform a quick entropy poll 
...
static int preOpenSocket( INOUT NET_STREAM_INFO *netStream, 
	...
	BOOLEAN isDgramSocket = TEST_FLAG( netStream->nFlags, 
									   STREAM_NFLAG_DGRAM ) ? TRUE : FALSE;
