...
static BOOLEAN sanityCheckContentSig( IN const CONTENT_LIST *contentListPtr,
	...
	if( ( contentSigInfo->hashAlgo != CRYPT_ALGO_NONE && \
		...
		( contentSigInfo->hashAlgoParam != 0 && \
		  ( contentSigInfo->hashAlgoParam < 16 || \
	...
	if( contentSigInfo->attributeCursorEntry < CRYPT_ATTRIBUTE_NONE || \
		contentSigInfo->attributeCursorEntry >= CRYPT_ATTRIBUTE_LAST )
		{
...
BOOLEAN sanityCheckContentList( IN const CONTENT_LIST *contentListPtr )
	...
	if( contentListPtr->type <= CONTENT_NONE || \
		contentListPtr->type >= CONTENT_LAST || \
