...
CHECK_RETVAL_BOOL STDC_NONNULL_ARG( ( 1 ) ) \
BOOLEAN isEmptyData( IN_BUFFER_C( 8 ) const BYTE *data,
...
BOOLEAN pointerBoundsCheck( IN_OPT const void *data,
	...
	/* If there's no object data present, there's nothing to check, although 
	...
	/* If there's no inner data present, there's nothing to check */
	if( innerData == NULL )
		{
	...
	/* Make sure that the inner object is contained within the outer 
	   object */
	if( innerData < data || \
...
/* Delay by a small random amount, used to dither the results of (failed) 
   ...
   which will be a pain to diagnose for users.  3s seems to be a good 
   tradeoff between dithering operations and slowing down an attacker while 
   not adding an excessive delay that looks like it's caused by something 
   else */

static void randomDelay( const int maxDelay )
	{
	int delayTime = getRandomInteger();

	ENSURES_V( maxDelay >= 100 && maxDelay <= 5000 );

	/* Use a delay from 0.01s to maxDelay.  getRandomInteger() can return 
	   zero for a shouldn't-occur error condition, this is converted into a 
	   small nonzero wait alongside an actual zero value */
	delayTime %= ( maxDelay + 1 );
	if( delayTime < 5 )
		delayTime = 5;
