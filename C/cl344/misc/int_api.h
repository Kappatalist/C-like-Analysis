...
#define HWCAP_FLAG_NONE		0x000	/* No special HW capabilities */
#define HWCAP_FLAG_RDTSC	0x001	/* x86 RDTSC instruction support */
#define HWCAP_FLAG_XSTORE	0x002	/* VIA XSTORE instruction support */
#define HWCAP_FLAG_XCRYPT	0x004	/* VIA XCRYPT instruction support */
#define HWCAP_FLAG_XSHA		0x008	/* VIA XSHA instruction support */
#define HWCAP_FLAG_MONTMUL	0x010	/* VIA bignum instruction support */
#define HWCAP_FLAG_TRNG		0x020	/* AMD Geode LX TRNG MSR support */
#define HWCAP_FLAG_AES		0x040	/* Intel AES instruction support */
#define HWCAP_FLAG_RDRAND	0x080	/* Intel RDRAND instruction support */
#define HWCAP_FLAG_RDSEED	0x100	/* Intel RDSEED instruction support */
#define HWCAP_FLAG_MAX		0x1FF	/* Maximum possible flag value */
...
typedef CHECK_RETVAL \
		const DATAPTR ( *DATAPTR_GETATTR_FUNCTION ) \
...
/* Variables and functions to support logging of keyset data to a JSON 
   record */
...
typedef enum { 
	JSON_NONE,				/* No JSON type */
	JSON_BEGIN,				/* Start JSON records */
	JSON_BOOLEAN,			/* Boolean value */
	JSON_NUMERIC,			/* Numeric value */
	JSON_STRING,			/* String value */
	JSON_STRING_HEX,		/* String value as hex digits */
	JSON_OBJECT_START,		/* Start of object, "label { " */
	JSON_OBJECT_END,		/* End of object, " } " */
	JSON_ARRAY_START,		/* Start of array, "label [ " */
	JSON_ARRAY_ITEM,		/* Entry in array */
	JSON_ARRAY_END,			/* End of array, " ] " */
	JSON_LAST				/* Last possible JSON type */
	} JSON_TYPE;

/* Add JSON-formatted data to the JSON record.  The wrapper macros always 
   return CRYPT_ERROR_OVERFLOW on error since running out of buffer space is 
   the only user-visible problem that can occur */

CHECK_RETVAL_BOOL \
BOOLEAN jsonAdd( IN_ENUM( JSON ) const JSON_TYPE type, 
				 IN_STRING_OPT const char *label, 
				 IN_LENGTH_Z const int value, 
				 IN_STRING_OPT const char *string );
#define JSON_BEGIN_DATA() \
		if( !( jsonAdd( JSON_BEGIN, NULL, 0, NULL ) ) ) \
			return( CRYPT_ERROR_OVERFLOW )
#define JSON_END_DATA	JSON_ADD_ARRAY_END
#define JSON_ADD_BOOLEAN( label, boolean ) \
		if( !( jsonAdd( JSON_BOOLEAN, label, boolean, NULL ) ) ) \
			return( CRYPT_ERROR_OVERFLOW )
#define JSON_ADD_NUMERIC( label, value ) \
		if( !( jsonAdd( JSON_NUMERIC, label, value, NULL ) ) ) \
			return( CRYPT_ERROR_OVERFLOW )
#define JSON_ADD_STRING( label, string ) \
		if( !( jsonAdd( JSON_STRING, label, 0, string ) ) ) \
			return( CRYPT_ERROR_OVERFLOW )
#define JSON_ADD_STRING_HEX( label, string, length ) \
		if( !( jsonAdd( JSON_STRING_HEX, label, length, string ) ) ) \
			return( CRYPT_ERROR_OVERFLOW )
#define JSON_ADD_OBJECT() \
		if( !( jsonAdd( JSON_OBJECT_START, NULL, 0, NULL ) ) ) \
			return( CRYPT_ERROR_OVERFLOW )
#define JSON_ADD_OBJECT_END() \
		if( !( jsonAdd( JSON_OBJECT_END, NULL, 0, NULL ) ) ) \
			return( CRYPT_ERROR_OVERFLOW )
#define JSON_ADD_ARRAY( label ) \
		if( !( jsonAdd( JSON_ARRAY_START, label, 0, NULL ) ) ) \
			return( CRYPT_ERROR_OVERFLOW )
#define JSON_ADD_ARRAY_ITEM( item, itemLength ) \
		if( !( jsonAdd( JSON_ARRAY_ITEM, item, itemLength, NULL ) ) ) \
			return( CRYPT_ERROR_OVERFLOW )
#define JSON_ADD_ARRAY_END() \
		if( !( jsonAdd( JSON_ARRAY_END, NULL, 0, NULL ) ) ) \
			return( CRYPT_ERROR_OVERFLOW )
#else

#define JSON_BEGIN_DATA()
#define JSON_END_DATA()
#define JSON_ADD_BOOLEAN( label, boolean )
#define JSON_ADD_NUMERIC( label, value )
#define JSON_ADD_STRING( label, string )
#define JSON_ADD_STRING_HEX( label, string, length )
