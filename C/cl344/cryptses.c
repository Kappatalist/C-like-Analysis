...
BOOLEAN sanityCheckSession( const SESSION_INFO *sessionInfoPtr )
	...
	if( sessionInfoPtr->type <= CRYPT_SESSION_NONE || \
		sessionInfoPtr->type >= CRYPT_SESSION_LAST || \
		sessionInfoPtr->version < 0 || \
		sessionInfoPtr->version > 5 ) 
		{
	...
	if( sessionInfoPtr->authResponse < AUTHRESPONSE_NONE || \
		sessionInfoPtr->authResponse >= AUTHRESPONSE_LAST )
		{
	...
	if( !checkObjectHandle  ( sessionInfoPtr->transportSession ) )
		{
		DEBUG_PUTS(( "sanityCheckSession: Transport session handle" ));
		return( FALSE );
		}
...
static BOOLEAN sanityCheckProtocolInfo( const PROTOCOL_INFO *protocolInfo )
	...
	if( protocolInfo->flags < SESSION_FLAG_NONE || \
		protocolInfo->flags > SESSION_FLAG_MAX )
		{
