...
int testTimingAttack( void )
	...
	for( i = 0; i < 200; i++ )
		...
		if( cryptStatusError( status ) )
			{
			dprintf( outputStream, "cryptImportKey() failed with status %s, "
					 "line %d.\n", status, __LINE__ );
			return( FALSE );
			}
		times[ i ] = timeVal;
			...
			case IS_NUMERIC:
				...
				if( cryptStatusError( status ) || \
					...
					fprintf( outputStream, "Read back of numeric attribute "
							 "for entry %d, field ID %d, value %d,\n"
							 "  failed, line %d.\n", i + 1, 
							 certData[ i ].type, certData[ i ].numericValue,
							 lineNo );
			...
			case IS_STRING:
				...
				if( cryptStatusError( status ) || \
					...
					fprintf( outputStream, "Read back of string attribute "
							 "for entry %d, field ID %d, failed, line %d.\n", 
							 i + 1, certData[ i ].type, lineNo );
					status = CRYPT_ERROR_FAILED;
					}
				break;
				}
