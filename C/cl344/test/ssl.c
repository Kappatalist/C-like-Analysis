...
/* There are various servers running that we can use for testing, the 
   following remapping allows us to switch between them.  Notes:
	...
	Server 2-4: Generic test servers at google.com (formerly amazon.com), 
			  ...
			  2014 Amazon disabled SSLv3 on all of its servers, Paypal 
			  disabled it in early 2015, and Google disabled in mid 2016,
			  which is what the Amazon/Google servers were used to test, so
			  now we use any number of US banks, which will hopefully keep
			  supporting insecure modes more or less forever, see the
			  entries around #45 for samples.  In addition as part of the 
			  deprecation of SHA-1 in early 2016 Comodo, used by the Red 
			  Hat server, switched its CA certs to SHA-384, so we need to 
			  enable the use of the extended SHA-2 hash functions to deal 
			  with this.
...
/* Various servers used for STARTTLS/STLS/AUTH TLS testing.  Notes:
	...
	Server 11: POP: pop.gmail.com:995 (74.125.28.109).  Formerly STARTTLS,
			   now requires a direct SSL connect.
...
static int connectSSLTLS( const CRYPT_SESSION_TYPE sessionType,
	...
	if( isServer )
		{
		puts( "  Warning: Session cache is active, subsequent sessions won't "
			  "use\n  the full handshake." );
		}
	...
	if( isServer )
		...
		if( testType == SSL_TEST_STARTTLS )
			...
			/* Testing this fully requires a lot of OS-specific juggling so 
			   unless we're running under Windows or Linux we just supply 
			   the handle to stdin, which will return a read/write error 
			   during the connect.  This checks that the handle has been 
			   assigned corectly without requiring a lot of OS-specific 
			   socket handling code.  Otherwise, we use a (very cut-down) 
			   set of socket calls to set up a minimal socket.  Since 
			   there's very little error-checking done, we don't treat a 
			   failure as fatal */
#ifdef USE_LOCAL_SOCKETS
  #ifdef __WINDOWS__
			WSADATA wsaData;
...
dualThreadContinue:
	...
	if( cryptStatusError( status ) )
		...
		if( ( testType == SSL_TEST_NORMAL ) && ( version == 0 ) )
			{
			puts( "  (This test checks SSLv3 functionality, which fewer and "
	...
	if( isServer && testType == SSL_TEST_PSK )
		...
		if( length < 8 || memcmp( userNameBuffer, "[Resumed", 8 ) )
			{
			printf( "SVR: User name was '%s', should have been an "
					"indication that\n     the session was resumed, line "
  ...
  #pragma convert( 819 )
			...
			const char serverReply[] = \
				"HTTP/1.0 200 OK\n"
				"Date: Fri, 7 June 2016 20:02:07 GMT\n"
