...
/* Tables mapping SSHv2 algorithm names to cryptlib algorithm IDs, in 
   ...
   SSH has multiple ways to specify the same keyex algorithm, either
   negotiated via "diffie-hellman-group-exchange-sha1" or explicitly as
   "diffie-hellman-group14-sha1".  In order to distinguish between DH + SHA1 
   and DH + SHA1, we specify the hash algorithm for the latter as 
   CRYPT_ALGO_RESERVED1 in order to know that we have to send the peer the
   group14 form of the suite name and not the group-exchange form */
...
static int readAlgoStringEx( INOUT STREAM *stream, 
	...
	LOOP_MED( ( stringPos = 0, noStrings = 0 ),
			  stringPos <= stringLen - SSH2_MIN_ALGOID_SIZE && \
				!foundMatch && noStrings < MAX_NO_SUBSTRINGS, 
...
int writeAlgoStringEx( INOUT STREAM *stream, 
					   ...
					   const BOOLEAN useSecondAlgoGroup )
	{
...
int writeAlgoString( INOUT STREAM *stream, 
	...
	return( writeAlgoStringEx( stream, algo, CRYPT_ALGO_NONE, FALSE ) );
	}
...
static int processBodyFunction( INOUT SESSION_INFO *sessionInfoPtr,
	...
	*readInfo = READINFO_FATAL;
	...
	ENSURES( payloadLength >= 0 && \
			 ...
			 /* pPL check must be '<' rather than '<=' because of the stripped 
			    padding */
