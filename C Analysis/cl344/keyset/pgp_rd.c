...
static int readRSAKeyComponents( INOUT STREAM *stream, 
	...
	*pubKeyComponentLength = 0;
	...
	if( cryptStatusError( status ) )
		return( status );
	totalLength += length;
	JSON_ADD_NUMERIC( "keySize", bytesToBits( length - UINT16_SIZE ) );
	...
	*pubKeyComponentLength = 0;
	...
	switch( pgpPkcAlgo )
		...
		case PGP_ALGO_ELGAMAL:
			JSON_ADD_STRING( "algorithm", "Elgamal" );
		...
		case PGP_ALGO_ECDH:
			JSON_ADD_STRING( "algorithm", "ECDH" );
		...
		case PGP_ALGO_ECDSA:
			JSON_ADD_STRING( "algorithm", "ECDSA" );
...
static int readUserID( INOUT STREAM *stream, 
	...
	if( pgpInfo != NULL && pgpInfo->lastUserID < MAX_PGP_USERIDS )
		...
		JSON_ADD_ARRAY_ITEM( dataPtr, packetLength );
		}
...
static int readKey( INOUT STREAM *stream, 
	...
	JSON_ADD_BOOLEAN( "isPublicKey", isPublicKey );
	...
	if( cryptStatusError( status ) )
		return( status );
	JSON_ADD_STRING_HEX( "time", packetHeader + 4, 4 );
	...
	memcpy( keyInfo->openPGPkeyID, hash + hashSize - PGP_KEYID_SIZE,
			PGP_KEYID_SIZE );
	JSON_ADD_STRING_HEX( "keyID", keyInfo->openPGPkeyID, PGP_KEYID_SIZE );
...
static int processPacketGroup( INOUT STREAM *stream, 
	...
	if( cryptStatusError( status ) )
		...
		if( status == OK_SPECIAL )
			{
			/* OK_SPECIAL means that we encountered something that we 
			   couldn't process but that isn't an error, so we'll be
...
static int processKeyringPackets( INOUT STREAM *stream, 
	...
	*unhandledDataPresent = FALSE;
	...
	LOOP_MAX_INITCHECK( ( moreData = TRUE, bufEnd = 0 ),
		...
		JSON_ADD_OBJECT();
		JSON_ADD_NUMERIC( "start", filePos );
		JSON_ADD_NUMERIC( "length", length );
		if( keyMatchInfo == NULL )
			{
