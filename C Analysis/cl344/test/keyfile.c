...
int testGetPGPPublicKey( void )
	...
	if( !getPGPPublicKey( KEYFILE_OPENPGP_AES, NULL, "OpenPGP (GPG/AES-256 key)" ) )
		return( FALSE );
	if( !getPGPPublicKey( KEYFILE_OPENPGP_AES_KEYID, NULL, "OpenPGP (GPG/AES-256 key) by keyID" ) )
		return( FALSE );
...
#else
	fputs( "Skipping alternative key file read since 3DES isn't available.\n", 
		   outputStream );
#endif /* USE_3DES */
...
#else
	fputs( "Skipping alternative key file test since 3DES isn't available.\n", 
		   outputStream );
	return( TRUE );
#endif /* USE_3DES */
	}
...
static int writeSingleStepFileCert( const CRYPT_ALGO_TYPE cryptAlgo,
	...
	if( cryptStatusError( status ) )
		...
		if( useAltKeyFile && status == CRYPT_ERROR_NOTAVAIL )
			{
			/* If the format isn't supported, this isn't a problem */
			fputs( "Single-step update to alternative key file skipped.\n", 
				   outputStream );
			return( CRYPT_ERROR_NOTAVAIL );
			}
		fprintf( outputStream, "cryptKeysetOpen() failed with error "
