...
/* Map of common error codes to strings.  
   ...
   specified via the cryptSpecificCode member */
...
static const SOCKETERROR_INFO socketErrorInfo[] = {
	...
	{ IP_ERR_WOULD_BLOCK, CRYPT_OK, TRUE,
		"IP_ERR_WOULD_BLOCK: Socket is in non-blocking state and the current operation would block the socket if not in non-blocking state", 129 },
	...
	{ IP_ERR_PIPE, CRYPT_OK, TRUE,
		"IP_ERR_PIPE: Socket is not in the correct state for this operation", 66 },
	...
	{ IP_ERR_SHUTDOWN, CRYPT_ERROR_COMPLETE, TRUE,
		"IP_ERR_SHUTDOWN: Connection has been closed as soon as all data has been received upon a FIN request", 100 },
	{ IP_ERR_NO_PROTO_OPT, CRYPT_OK, TRUE,
		"IP_ERR_NO_PROTO_OPT: Unknown option for setsockopt() or getsockopt()", 68 },
	...
	{ IP_ERR_ADDR_NOT_AVAIL, CRYPT_ERROR_NOTFOUND, TRUE,
		"IP_ERR_ADDR_NOT_AVAIL: No known path to send to the specified address", 69 },
	{ IP_ERR_ADDR_IN_USE, CRYPT_OK, TRUE,
		"IP_ERR_ADDR_IN_USE: Socket already has a connection to this address and port or is already bound to this address", 112 },
	...
	{ IP_ERR_NOT_SOCK, CRYPT_OK, TRUE,
		"IP_ERR_NOT_SOCK: Socket has already been opened or has already been closed", 74 },
	...
	{ IP_ERR_NOBUFFER, CRYPT_ERROR_MEMORY, TRUE,
		"IP_ERR_NOBUFFER: System error: No internal buffer available for the requested operation", 87 },
	{ IP_ERR_RESOURCE, CRYPT_ERROR_MEMORY, TRUE,
		"IP_ERR_RESOURCE: System error: Not enough free resources available for the requested operation", 94 },
...
/* Get and set the low-level error information from a socket- and host-
   ...
   but this is only available under Vista and newer and requires playing 
   with COM objects, the significant extra complexity caused by this isn't 
   worth the tiny additional level of granularity that we might gain in 
   reporting errors.  In any case the NLM functionality seems primarily 
   intended for interactively obtaining information before any networking 
   actions are initiated ("Do we currently have an Internet connection?") 
   rather than diagnosing problems afterwards ("What went wrong with the 
   attempt to initiate an Internet connection?") */
...
int diagnoseConnectionProblem( INOUT NET_STREAM_INFO *netStream, 
	...
	diagnosticNetStream.iTransportSession = CRYPT_ERROR;
	...
	if( getIPVersion( buffer[ PACKET_OFFSET_IPVERSION ] ) == 4 )
		...
		{
		/* It wasn't IPv4, check for an IPv6 packet (which in the current 
		   case is rather unlikely given that we explicitly send IPv4
		   pings) and make sure that we've got enough data for an ICMP 
		   reply:
		...
		/* Now make sure that we've got an ICMP reply.  The latter serves 
		   ...
		   The presence of these in already-rare IPv6 packets should be even
		   more rare given how many systems will drop any IPv6 packets that
		   contain them (for example RFC 7872 reports packet drop rates of 
		   over 50% for some extesion header types) */
